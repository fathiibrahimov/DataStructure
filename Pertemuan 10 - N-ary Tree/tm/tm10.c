		...
		baru->kontainer = c; baru->child = NULL;
			...
			if(root->child == NULL){
				...
				if(root->child->sibling == NULL){
					...
					baru->sibling = root->child; last->sibling = baru;
				}
			}
	}
}
...
void printTreePreOrder2(simpul *root){
	if(root != NULL){
